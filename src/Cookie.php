...
class Cookie
{
    ...
    public static function init(array $config = [])
    {
        ...
        if (!empty(static::$config['httponly']) && PHP_SESSION_ACTIVE != session_status()) {
    ...
    public static function set($name, $value = '', $option = null)
    {
        ...
        if ($config['setcookie']) {
            static::setCookie($name, $value, $expire, $config);
        }
    ...
    {
        $prefix = !is_null($prefix) ? $prefix : static::$config['prefix'];
        $name = $prefix . $name;
    ...
    public static function delete($name, $prefix = null)
    {
        ...
        if ($config['setcookie']) {
            static::setcookie($name, '', $_SERVER['REQUEST_TIME'] - 3600, $config);
        }
        ...
        if ($prefix) {
            ...
            foreach ($_COOKIE as $key => $val) {
                if (0 === strpos($key, $prefix)) {
                    if ($config['setcookie']) {
                        static::setcookie($key, '', $_SERVER['REQUEST_TIME'] - 3600, $config);
                    }
                    unset($_COOKIE[$key]);
                }
            }
